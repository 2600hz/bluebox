...
/**
 * plugins.php - FreePbx plugins helper
 *
 * This helper provides methods that abstract plugin management in FreePbx. The idea is to abstract plugin events
 * for two reasons:
 ...
 *
 * See the manual at http://freepbx.org/v3/trac/ to learn more on how to utilize the features of FreePbx's plugin system.
 *
...
class plugins
{
    ...
    public static function register($url, $action, array $callback)
    {
        ...
        if (class_exists($callback[0]) and !is_subclass_of($callback[0], 'FreePbx_Plugin')) {
            throw new Exception('Tried to use class ' . $callback[0] . ' as a plug-in, but it does not extend FreePbx_Plugin. All plug-ins must extend FreePbx_Plugin.');
        }
