...
class message
{
    ...
    public static function set($text = '', $options = array())
    {
        ...
        // append this message into any existing session freepbx_messages
        $currentMessages = Session::instance()->get('freepbx_message', array());
        $currentMessages[] = $options;
        Session::instance()->set('freepbx_message', $currentMessages);
    ...
    public static function render($displayOnlyType = array(), $options = array())
    {
        ...
        // get the messages
        $flashMessages = Session::instance()->get_once('freepbx_message', array());
        ...
        Session::instance()->set('freepbx_message', $flashMessages);

        // if we are doing this inline then echo it out here and now
        if (!empty($options['inline'])) {
            if (!empty($html)) echo implode(' ', $html);
            if (!empty($growl)) { jquery::addPlugin('growl'); jquery::evalScript(implode(' ', $growl)); }
        }
