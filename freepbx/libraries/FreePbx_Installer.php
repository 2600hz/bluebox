...
/**
 * FreePbx_Configure.php - Classes to manage module installation, removal, and activation.
 ...
 */
class FreePbx_Installer
{
    //class FreePbx_PackageManager?

    /**
     * These are a list of category name constants for use in
     * FreePbx_Configure $category
     */
    ...
    public static function init()
    {
        ...
        if (empty(FreePbx_Installer::$instance)) {
            FreePbx_Installer::$instance = new FreePbx_Installer();
        }
    }
    /**
     * This function scans the freepbx module path for configure files and returns a list
     * of packages.
     *
     * @return array
    ...
    public static function listPackages($filter = array() , $ignoreDB = false)
    {
        ...
        foreach($configurations as $configuration) {
            ...
            // Check if we found a FreePbx_Configure class
            if ($foundClass && is_subclass_of($foundClass, 'FreePbx_Configure')) {
        ...
        foreach(self::$configurations as $class => $configuration) {
            ...
            if (!defined('Freepbx_Installer::' . $packageVars['type'])) $packageVars['type'] = FreePbx_Installer::TYPE_DEFAULT;
            if ($packageVars['type'] == FreePbx_Installer::TYPE_DEFAULT) {
    ...
    private static function _fulfillRequired($packages, $name)
    {
        ...
        foreach($requirements as $required => $requiredVersion) {
            ...
            if ($requirement == 'core') {
                $coreVersion = FreePbx_Controller::$version;
                if (!self::_compareVersions($requiredVersion, $coreVersion)) self::_setError('This module is incompatable with FreePbx version ' . $coreVersion , $name);
                continue;
            }
    ...
    public function installCore(&$packages) {
        $core['core'] = array (
            'version' => FreePbx_Controller::$version,
            'packageName' => 'core',
            'author' => 'FreePBX Team',
            'vendor' => 'FreePBX',
            'license' => 'MPL',
