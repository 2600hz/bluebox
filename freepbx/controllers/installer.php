...
/**
 * installer.php - Auto-installer. Installs FreePbx and initializes modules.
 *
 ...
 */
class Installer_Controller extends FreePbx_Controller
{
    /**
    ...
    *************************************************************************/
    ...
    private function welcome()
    {
        ...
        $this->template->title = i18n('Welcome to FreePBX %s Setup Wizard', FreePbx_Controller::$version)->sprintf()->s();
     ...
     * if the environment is capable of supporting the freepbx dependency each method is designed to check.
     *
     * @return subview
     */
    ...
    private function processConfigure()
    {
        ...
        try {
            ...
            try {
                 ...
                 */
                if (!is_string($response['FreePbxDB'])) {
                    $conn = $manager->getCurrentConnection();
                    $conn->rethrowException($response['FreePbxDB'], $conn);
                }
            }
            catch(Exception $e) {
                // We cant resolve this issue without the user
    ...
    private function telephony()
    {
        ...
        $packages = FreePbx_Installer::listPackages(FreePbx_Installer::TYPE_DRIVER);
        $drivers = array(
            'none' => 'None'
        );
    ...
    private function choosePackages()
    {
        ...
        $telDrivers = FreePbx_Installer::listPackages(FreePbx_Installer::TYPE_DRIVER);
        ...
        $packages = FreePbx_Installer::listPackages(array() , $ignoreDB);
        $install = array();
        ...
        FreePbx_Installer::warningSort($packages);
        FreePbx_Installer::errorSort($packages);
        ...
        }else if (!empty(FreePbx_Installer::$warnings)) {
            message::set('You may continue installation however some packages have potential issues.', 'info');
            $this->session->set('installer.warnings', FreePbx_Installer::$warnings);
        }
        // Pass all the warnings and errors onto the view
        $subview->currentDriver = $currentDriver;
        $subview->packageList = $this->createPackageList($packages, FreePbx_Installer::$errors, FreePbx_Installer::$warnings);
        return $subview;
    }
    private function processChoosePackages()
    {
        ...
        // Test the dependencies of all the modules the user wants to install
        FreePbx_Installer::checkDependencies($packages, $install);
        // Ensure there are no new warnings
        $old_warnings = $this->session->get('installer.warnings', array());
        if (!arr::array_compare_recursive(FreePbx_Installer::$warnings, $old_warnings)) return FALSE;
        // If all the dependencies are met let the user continue
        return empty(FreePbx_Installer::$errors);
    }
    /**
    ...
    private function processDoInstall()
    {
        ...
        if ($result != false && empty(FreePbx_Installer::$errors)) {
            ...
            if (empty($this->template->error)) {
                message::set('Install failed with errors!'
                    .'<div>'
                    .arr::arrayToUL(FreePbx_Installer::$errors, array(), array('class' => 'error_details', 'style' => 'text-align:left;'))
                    .'</div>'
                );                
    ...
    private function finalize()
    {
        ...
        $this->template->allowNext = false;
        self::_resetWizard();
        $this->session->delete('freepbx_message');
    ...
    *************************************************************************/
    ...
    private function _freshInstall()
    {
        ...
        $initMethods = get_class_methods('FreePbx_Initialize');
        ...
        try {
            $result = FreePbx_Installer::processActions($packages, $install);
            Kohana::config_set('core.log_threshold', $this->log_threshold);
            return $result;
        } catch(Exception $e) {
    ...
    *************************************************************************/
    ...
    /**
     * Ensures that open_basedir is null or the freepbx directory
     *
     * @return results array
     */
    private function _checkBaseDir()
    {
        $result = array(
            ...
            'fail_msg' => __('Your open_basedir must be null or contain the freepbx directory') ,
            'pass_msg' => __('Pass') ,
            'result' => false,
            'required' => true
        );
    ...
    /**
     * Ensures FreePbx application directory exists
     *
     * @return results array
     */
    ...
    {
        $result = array(
            'name' => __('FreePBX Directory') ,
            'fail_msg' => __('The configured Freepbx directory does not exist or does not contain required files.') ,
            'pass_msg' => __('Pass') ,
