 ...
 *
 * $Id: Kohana.php 3881 2009-01-09 19:59:03Z jheathco $
 *
 * @package    Core
 * @author     Kohana Team
...
final class Kohana {
	...
	public static function log($type, $message)
	{
            ...
            // TODO: Get this out of Doctrine. Starting here, we hacked this in. It should be removed.
            // This is also very expensive!
                ...
                if ( ! IN_PRODUCTION ) {
                    ...
                    if (isset($backtrace[0]['file'])) {
                        $info .= substr($backtrace[0]['file'], strlen(DOCROOT)) .  '[' . $backtrace[0]['line'] . '] ';
                    }

                    if (isset($backtrace[1]['class'])) {
                        $info .= '(' . $backtrace[1]['class'] . ') ';
                    }

                    if (isset($backtrace[1]['function'])) {
                        $info .= $backtrace[1]['function'] . ': ';
                    }
                } else {
                    $info = '';
                }

                $message = array(date('Y-m-d H:i:s P'), $type, $info . $message);
		...
		foreach ($trace as $entry)
		{
			...
			if (isset($entry['args']) AND is_array($entry['args']))
			{
				...
				while ($arg = array_shift($entry['args']))
				{
                    ...
                    // Doctrine keeps overwhelming the backtrace procedure here because it has too much data and kills memory.
                    // So we're suppressing this for now
