...
class FreeSwitch_NumberContext_Driver extends FreeSwitch_Base_Driver {
    ...
    public static function set($obj)
    {
        ...
        //Kohana::log('debug', 'Before create extension...' . $xml->saveXml());
        dialplan::start('context_' . $obj->context_id);
        //Kohana::log('debug', 'After create extension...' . $xml->saveXml());

        // Reference to our XML document, context and extension. Creates the extension & context if does not already exist
        $xml = FreeSwitch::createExtension('number_' . $obj->Number->number_id, 'main', 'context_' . $obj->context_id);
        ...
        if ($obj->Number->class_type) {
            ...
            if (class_exists($destinationDriverName, TRUE)) {
                ...
                kohana::log('debug', 'EVAL ' . $destinationDriverName . '::dialplan($obj->Number);');
    ...
    public static function delete($obj)
    {
        ...
        $xml->setXmlRoot(sprintf('//document/section[@name="dialplan"]/context[@name="context_' . $obj->context_id . '"]/extension[@name="%s"]', 'main_number_' . $obj->Number->number_id));
