...
class FreeSwitch_SimpleRouteContext_Driver extends FreeSwitch_Base_Driver
{
    public static function set($obj)
    {
        ...
        // If you feel so inspired, the real routing module is spec'ed out here: http://www.freepbx.org/v3/wiki/RoutingModules
        ...
        if ($obj->SimpleRoute->emergency && $pattern = simplerouter::getOutboundPattern('emergency', 'freeswitch')) {
            ...
            $emergency = '/condition[@field="destination_number"][@expression="' . $pattern . '"][@freepbx="emergency"]';

            // TODO: Allow for a specific Caller ID on E911 calls. That would go here.
            $xml->update($emergency . '/action[@application="set"][@freepbx="prepend"]{@data="prepend=' . $obj->SimpleRoute->emergency_prepend . '"}');
            $xml->update($emergency . '/action[@application="set"][@freepbx="cid_name"]{@data="effective_caller_id_name=' . $obj->SimpleRoute->caller_id_name . '"}');
            $xml->update($emergency . '/action[@application="set"][@freepbx="cid_number"]{@data="effective_caller_id_number=' . $obj->SimpleRoute->caller_id_number . '"}');

            // If a Caller ID module is installed and caller ID is set, use it
            // TODO: Integrate this into the plugin
            $caller_id = '/condition[@field="${outbound_caller_id_number}"][@expression="^.+$"][@break="never"][@freepbx="caller_id"]';
            ...
            $dummy = '/condition[@field="destination_number"][@expression="' . $pattern . '"][@freepbx="global"]';
        ...
        if ($obj->SimpleRoute->international && $pattern = simplerouter::getOutboundPattern('international', 'freeswitch')) {
            ...
            $international = '/condition[@field="destination_number"][@expression="' . $pattern .'"][@freepbx="international"]';

            // TODO: Allow for a specific Caller ID on E911 calls. That would go here.
            $xml->update($international . '/action[@application="set"][@freepbx="prepend"]{@data="prepend=' . $obj->SimpleRoute->international_prepend . '"}');
            $xml->update($international . '/action[@application="set"][@freepbx="cid_name"]{@data="effective_caller_id_name=' . $obj->SimpleRoute->caller_id_name . '"}');
            $xml->update($international . '/action[@application="set"][@freepbx="cid_number"]{@data="effective_caller_id_number=' . $obj->SimpleRoute->caller_id_number . '"}');
        ...
        if ($obj->SimpleRoute->local && $pattern = simplerouter::getOutboundPattern('local', 'freeswitch')) {
            ...
            $domestic = '/condition[@field="destination_number"][@expression="' . $pattern .'"][@freepbx="domestic"][@break="never"]';

            $xml->update($domestic . '/action[@application="set"][@freepbx="prepend"]{@data="prepend=' . $obj->SimpleRoute->local_prepend . '"}');
            $xml->update($domestic . '/action[@application="set"][@freepbx="cid_name"]{@data="effective_caller_id_name=' . $obj->SimpleRoute->caller_id_name . '"}');
            $xml->update($domestic . '/action[@application="set"][@freepbx="cid_number"]{@data="effective_caller_id_number=' . $obj->SimpleRoute->caller_id_number . '"}');
            ...
            if ($pattern = simplerouter::getOutboundPattern('short', 'freeswitch')) {
                ...
                $local = '/condition[@field="destination_number"][@expression="' . $pattern .'"][@freepbx="local"]';

                $xml->update($local . '/action[@application="set"][@freepbx="prepend"]{@data="prepend=' . $obj->SimpleRoute->local_prepend . '"}');
                $xml->update($local . '/action[@application="set"][@freepbx="cid_name"]{@data="effective_caller_id_name=' . $obj->SimpleRoute->caller_id_name . '"}');
                $xml->update($local . '/action[@application="set"][@freepbx="cid_number"]{@data="effective_caller_id_number=' . $obj->SimpleRoute->caller_id_number . '"}');
            ...
            //$tollfree = '/condition[@field="destination_number"][@expression="^(18(0{2}|8{2}|7{2}|6{2})\d{7})$"][@freepbx="local"]';
