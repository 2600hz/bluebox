...
<div id="timeofday_update_form" class="update timeofday" style="width: 100%; overflow-x: scroll;">
	...
	<?php echo form::open_section('Timezone'); ?>
	    ...
	    <?php
	        ...
	        echo timezone::dropdown(array('id'=>'timezone','name'=>'timeofday[timezone]','class'=>'time_selector'), 
			empty($timeofday['timezone']) ? NULL : $timeofday['timezone'] );
	    ?>
	    </div>
        <?php echo form::close_section(); ?>
...
<script type="text/javascript" charset="utf-8">
    ...
    $('.time_selector').change(function () {
        ...
        if (text.length) {
            ...
            text = text.replace(/On Monday through Sunday between/, 'Between');
        } else {
	...
	tz=" in the "+tz.options[tz.selectedIndex].text+" timezone";

        var hours = $('#timeofday_time').val().split(";");
       
        $('#time_range_text').html(text + calculateHours(hours[0]) + " and " + calculateHours(hours[1]) + tz + ",<br /> route calls to a ");
