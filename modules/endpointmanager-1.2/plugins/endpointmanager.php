    ...
    public function addPluginData() {
        ...
        if((isset($device['plugins']['endpointdevice']['mac_address'])) && ($device['plugins']['endpointdevice']['mac_address'] != "")) {
            ...
            if($endpoint->mac_check_clean($device['plugins']['endpointdevice']['mac_address'])) {
                ...
                if (isset($device['User']['Location']['domain']['plugins']['sipinterface']) && !($device['User']['Location']['domain']['plugins']['sipinterface'] == '0'))
                {
                	$boundinterface = Doctrine::getTable('SipInterface')->findOneBysipinterface_id($device['User']['Location']['plugins']['sipinterface']);
                } else {
                	$boundinterface = Doctrine::getTable('SipInterface')->findOneByname(SipInterface::get_default());
                }
				if ($boundinterface)
					$sipinterfaceport = $boundinterface->port;
				else
					$sipinterfaceport = '5060';
				...
				$phone_info['host'] = $dns;
				$phone_info['port'] = $sipinterfaceport;
