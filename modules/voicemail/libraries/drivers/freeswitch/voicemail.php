...
class FreeSwitch_Voicemail_Driver extends FreeSwitch_Base_Driver {
    ...
    public static function preNumber()
    {
        ...
        if (!empty($base->foreign_id) && !empty($domain) && !empty($mailbox)) {
            ...
            $xml->update('/action[@application="set"][@bluebox="settingTimeout"]{@data="call_timeout=' . $timeout . '"}');
            ...
            $xml->update('/action[@application="set"][@bluebox="ring-xfer"]{@data="transfer_ringback=${' . $ringtype . '}"}');
        }
    }
